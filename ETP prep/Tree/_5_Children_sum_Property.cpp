...
public:
    ...
    bool Children_Sum(Node *curr){
        ...
        else if((curr->left->data )+(curr->right->data) == curr->data){
            return true;
        }
        else if((curr->left->data )+(curr->right->data) != curr->data){
            return false;
        }      
        return Children_Sum(curr->left)*Children_Sum(curr->right);
    }
};
